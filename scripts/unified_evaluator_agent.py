...
"""
"""
Updated evaluation pipeline with AutoGen agent integration

Key changes:
1. Added AutoGen agent management
2. Integrated RAG retrieval with agent workflows
3. Modified conversation handling for agent interactions
4. Maintained compatibility with existing evaluation metrics
"""
...
class TaskHandler:
    def get_agent_config(self, args) -> Dict[str, Any]:
        """Get AutoGen configuration for agents"""
        config_list = config_list_from_json(
            "OAI_CONFIG_LIST",
            filter_dict={"model": [args.model.replace("openai/", "")]} if args.model.startswith("openai") else None,
        )
        return {
    ...
    def create_agents(self, args, retriever=None):
        """Initialize AutoGen agents with RAG capabilities"""
        llm_config = self.get_agent_config(args)

        # Create assistant agent
        assistant = AssistantAgent(
            name="assistant",
            llm_config=llm_config,
            system_message="You are a helpful AI assistant. Respond thoughtfully and accurately."
        )

        # Create user proxy with retrieval capabilities
        if retriever:
            rag_agent = RetrieveUserProxyAgent(
                name="rag_proxy",
                retrieve_config={
                    "task": "qa",
                    "docs_path": self.get_rag_document_path(),
                    "chunk_token_size": 2000,
                    "model": args.model,
                    "client": retriever.ensemble_retriever
                },
                human_input_mode="NEVER",
                max_consecutive_auto_reply=3
            )
            return rag_agent, assistant
        else:
...
def perform_agent_inference(handler, conversations, args, retriever=None):
    """Execute inference using AutoGen agents"""
    all_responses = []
    token_usages = []

    for conv in tqdm(conversations, desc="Agent Processing"):
        user_proxy, assistant = handler.create_agents(args, retriever)

        try:
            # For RAG-enabled conversations
            if retriever:
                ...
                user_proxy.initiate_chat(perform_inference_and_check(
                    assistant,
                    message=conv[-1]["content"])
                )

            # Extract final response
            final_response = assistant.last_message()["content"]
            all_responses.append(final_response)

            # Track token usage
            if args.model.startswith("openai"):
                token_usages.append(assistant.client.last_usage())
            else:
                # vLLM token tracking
                token_usages.append({
                    "prompt_tokens": len(assistant.last_message().prompt_token_ids),
                    "completion_tokens": len(assistant.last_message().completion_token_ids)
                })
    ...
    return all_responses, token_usages
...
def perform_inference_and_check(handler: TaskHandler, temperatures, max_tokens, result_file, llm, system_prompt, args):
    """Updated inference-checking with AutoGen integration"""
    # Initialize RAG components
    rag_documents = handler.load_rag_documents()
    retriever = RagRetriever(rag_documents) if args.rag else None

    # Existing data loading remains
    results = handler.load_existing_results(result_file)
    train_data = handler.load_and_filter_dataset(args.start, args.end, args.split, args.source, args)
    ...
    # Run agent-based inference
    all_responses, token_usages = perform_agent_inference(
        handler, conversations, args, retriever
    )

    # Existing validation and result handling remains
